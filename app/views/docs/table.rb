...
class Views::Docs::Table < Views::Application
  ...
  def view_template
    ...
    div(class: "max-w-2xl mx-auto w-full py-10 space-y-8") do
      ...
      render Docs::VisualCodeExample.new(title: "Invoice Example", context: self) do
        <<~RUBY
      ...
      render Docs::VisualCodeExample.new(title: "Users Example", context: self) do
        <<~RUBY
          ...
          Table do
            TableCaption { "Top contributors" }
            TableHeader do
              TableRow do
                TableHead { "Name" }
                TableHead { "Commits" }
                TableHead(class: "text-right") { "Links" }
              end
            end
            TableBody do
              users.each do |user|
                TableRow do
                  TableCell(class: 'font-medium') do
                    Avatar do
                      AvatarImage(src: user.avatar_url, alt: "joeldrapper")
                      AvatarFallback { "JD" }
                    end
                  end
                  TableCell { user.commits }
                  TableCell(class: "text-right") do
                    Link(href: github_link(user), variant: :outline, size: :sm) do
